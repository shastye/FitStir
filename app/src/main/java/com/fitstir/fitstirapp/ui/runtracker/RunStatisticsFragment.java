...
import com.fitstir.fitstirapp.ui.runtracker.utilites.Methods;
...
public class RunStatisticsFragment extends Fragment {
    ...
    private TextView loc_Lat, loc_Lng,loc_Alt, loc_Bear, loc_Elapsed,
            loc_Speed, loc_Time, loc_Acc, calories,pace,runMessage, noRunMessage , dist;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mapImage = root.findViewById(R.id.route_Image);
        dist = root.findViewById(R.id.tv_Distance);
        ...
        if(viewRuns.getActions().getValue() == 1){
            ...
            String distance = String.valueOf(viewRuns.getTotalDistance().getValue());
            dist.setText(distance);
            Glide.with(requireActivity())
                    .load(viewRuns.getMapImage().getValue())
                    .into(mapImage);


            if(viewRuns.getAvgPace().getValue() > 0.2){
        ...
        if(viewRuns.getActions().getValue() == 2){
            ...
            String avgPace = String.valueOf(viewRuns.getAvgPace().getValue());
            pace.setText(avgPace);
            String time = String.valueOf(viewRuns.getTime().getValue());
                ...
                if(task.isSuccessful()){
                    ...
                    if(dataSnapshot.exists()){
                        ...
                        for (DataSnapshot data : dataSnapshot.getChildren()) {
                            ...
                            double cal = (double)data.child("burnedCalories").getValue();
                            timeEntry.add(new Entry(timeEntry.size(), (float)minutes));
                            distanceEntry.add(new Entry(distanceEntry.size(), (float) distance));
                            calEntry.add(new Entry(calEntry.size(), (float)cal));
                        }

                        Methods method = new Methods();
                        method.chartDesignMain(statChart, timeEntry, distanceEntry, calEntry);
